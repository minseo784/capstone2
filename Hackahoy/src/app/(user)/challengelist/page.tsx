...
export default function ChallengeListPage() {
  ...
  const filteredList = useMemo(() => {
    let list = problems;
    if (filter === 'SOLVED') list = problems.filter((p) => p.solved);
    else if (filter === 'UNSOLVED') list = problems.filter((p) => !p.solved);
    else if (filter !== 'ALL')
      list = problems.filter((p) => p.category === filter);
    return list;
  }, [filter, problems]);
